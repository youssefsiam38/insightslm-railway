#!/bin/bash
# insightslm-railway Kong entrypoint: mint the API keys, render the routes, start Kong.
set -uo pipefail
log()  { printf '[insightslm-kong] %s\n' "$*"; }
fail() { printf '[insightslm-kong] FATAL: %s\n' "$*" >&2; exit 1; }
...
for name in AUTH_HOST REST_HOST REALTIME_HOST STORAGE_HOST FUNCTIONS_HOST; do
...
keys=$(insightslm-mint-keys) || fail "could not mint the Supabase API keys"
...
log "gateway on :${PUBLIC_PORT} -> auth=${AUTH_HOST} rest=${REST_HOST} realtime=${REALTIME_HOST} storage=${STORAGE_HOST} functions=${FUNCTIONS_HOST}"
