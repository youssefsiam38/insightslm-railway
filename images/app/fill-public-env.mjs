// Write the deployment's public values into InsightsLM's built web app.
//
// Vite inlines every VITE_* variable into the browser bundle at build time.
...
const PLACEHOLDERS = {
  VITE_SUPABASE_URL: "https://insightslm-railway-placeholder-supabase-url.invalid",
  VITE_SUPABASE_ANON_KEY: "insightslm-railway-placeholder-supabase-anon-key",
};
const MARKER = "insightslm-railway-placeholder-";
...
const SHAPES = {
  VITE_SUPABASE_URL: /^https?:\/\/[A-Za-z0-9.-]+(:[0-9]{1,5})?$/,
  VITE_SUPABASE_ANON_KEY: /^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/,
};

const die = (msg) => {
  process.stderr.write(`[insightslm-app] FATAL: ${msg}\n`);
  process.exit(1);
};
...
function buildIndex(nextDir, pristineDir) {
  // The built site (index.html, assets/). Dependencies never carry a placeholder and are skipped.
...
function fill(appDir, pristineDir) {
  ...
  process.stdout.write(`[insightslm-app] public values written into ${files.length} built files\n`);
}

if (process.argv[2] === "--index") buildIndex(process.argv[3], process.argv[4]);
else fill(process.env.INSIGHTSLM_APP_DIR ?? "/app/dist", process.env.INSIGHTSLM_PRISTINE_DIR ?? "/opt/insightslm/pristine");
