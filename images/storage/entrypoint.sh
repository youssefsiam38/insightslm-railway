#!/bin/sh
# insightslm-railway storage entrypoint: mint the API keys, then run Supabase Storage unchanged.
set -eu
log()  { printf '[insightslm-storage] %s\n' "$*"; }
fail() { printf '[insightslm-storage] FATAL: %s\n' "$*" >&2; exit 1; }
...
keys=$(node /usr/local/lib/insightslm/mint-supabase-keys.mjs) || fail "could not mint the Supabase API keys"
