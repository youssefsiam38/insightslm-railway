#!/bin/bash
# insightslm-railway db entrypoint: validate, then hand over to Supabase's own entrypoint.
set -uo pipefail
log()  { printf '[insightslm-db] %s\n' "$*"; }
fail() { printf '[insightslm-db] FATAL: %s\n' "$*" >&2; exit 1; }
...
exec docker-entrypoint.sh postgres \
  ...
  -c pgsodium.getkey_script=/usr/local/bin/insightslm-pgsodium-getkey \
  -c vault.getkey_script=/usr/local/bin/insightslm-pgsodium-getkey \
  -c listen_addresses='*' \
